...
import { useParams, useLocation, useNavigate } from "react-router-dom"; // Link không được dùng, đã xóa
...
const Watch = () => {
  ...
  // Fetch film khi user hoặc slug thay đổi
   useEffect(() => {
    if (user && slug) { // Chỉ fetch khi có user và slug
      fetchFilm();
    } else if (!user && slug) {
        setError("Vui lòng đăng nhập để xem phim này.");
        navigate("/signin", { state: { from: location } }); // Chuyển hướng và lưu lại trang muốn đến
    }
  }, [user, slug, location, navigate]); 
  ...
  useEffect(() => {
    if (film?.movie?.video_url && videoRef.current && user) {
      ...
      const applyStartTime = () => {
        if (startTime > 0 && video.duration && video.currentTime < startTime * 60) { // Kiểm tra video.duration để chắc chắn metadata đã load
      ...
      const setupHls = (src) => {
        ...
        if (Hls.isSupported()) {
          ...
          hls.on(Hls.Events.MANIFEST_PARSED, (event, data) => {
            ...
            setCurrentLevel(hls.autoLevelEnabled ? -1 : hls.currentLevel); // Reflect current auto/manual level
            applyStartTime(); // Áp dụng thời gian bắt đầu sau khi manifest được parse
            if (video.paused) {
      ...
      const handleLoadedMetadata = () => {
        ...
        applyStartTime(); // Áp dụng thời gian bắt đầu khi metadata đã load
        if (video.paused && Hls.isSupported() && !hlsInstance?.media) { // Chỉ autoplay nếu không phải HLS đang quản lý
            // Hoặc nếu là HLS, autoplay đã được xử lý trong MANIFEST_PARSED
        } else if (video.paused && !Hls.isSupported() && video.src) { // Native HLS (Safari)
      ...
      video.addEventListener("ended", handlePause); // Set isPlaying to false on ended
      ...
      return () => {
        ...
        // Reset video src to prevent playing in background after component unmounts (important for SPA)
        if (videoRef.current) {
            videoRef.current.pause();
            videoRef.current.removeAttribute('src'); // remove the src
            videoRef.current.load(); // ask the browser to load nothing
        }
      };
    }
  ...
  const setVideoLevel = (levelIndex) => {
    ...
    setCurrentLevel(levelIndex); // Update UI immediately
  ...
  if (error && !film) { // Chỉ hiển thị lỗi toàn màn hình nếu không có film data
    return (
      <>
        <NavBar />
  ...
  if (!film?.movie?.video_url && film && !error) { 
      ...
      <main className="flex-grow">
        ...
        >
            ...
            <div
                ...
                className={`group relative mx-auto w-[98%] sm:w-[95%] md:w-[90%] lg:w-[85%] aspect-video flex flex-col justify-center items-center rounded-lg overflow-hidden shadow-2xl bg-black ${
            ...
            >
                ...
                <div className="w-full px-1 sm:px-2 mb-1.5 sm:mb-2">
                    ...
                    >
                    <div
                        className="bg-red-500 h-full rounded-full relative transition-all duration-100 ease-linear" // ease-linear for smoother updates
                ...
                <div className="w-full flex items-center justify-between px-1 sm:px-2">
                    ...
                    <div className="relative">
                        ...
                        {playbackRate === 1 ? "Tốc độ" : `${playbackRate}x`} <i className="bx bx-xs bxs-down-arrow align-middle"></i>
                        </button>
                        {showSpeedOptions && (
                        <div className="absolute bottom-full mb-1 right-0 bg-black/80 backdrop-blur-sm p-1.5 rounded z-20 whitespace-nowrap shadow-lg">
                            ...
                            <button key={rate} onClick={() => handleSetPlaybackRate(rate)}
                                className={`block w-full text-left text-xs sm:text-sm hover:bg-white/20 p-1 sm:p-1.5 rounded ${playbackRate === rate ? "font-bold text-red-400" : ""}`}>
                                {rate === 1 ? "1" : `${rate}x`}
                            </button>
                            ))}
                        </div>
                    ...
                    {qualityLevels.length > 0 && ( 
                        ...
                        {showQualityOptions && (
                            <div className="absolute bottom-full mb-1 right-0 bg-black/80 backdrop-blur-sm p-1.5 rounded z-20 whitespace-nowrap shadow-lg">
                            <button onClick={() => setVideoLevel(-1)}
                                className={`block w-full text-left text-s sm:text-sm font-semibold hover:bg-white/20 p-1 sm:p-1.5 rounded ${currentLevel === -1 ? "font-bold text-red-400" : ""}`}>
                                Auto
                            </button>
                            {qualityLevels.map((level, index) => (
                                <button key={index} onClick={() => setVideoLevel(index)}
                                className={`block w-full text-left text-s sm:text-sm hover:bg-white/20 p-1 sm:p-1.5 rounded ${currentLevel === index ? "font-bold text-red-400" : ""}`}>
        ...
        <div className="bg-[#06121e] pb-10 -mt-10 md:-mt-16 relative z-10"> {/* Negative margin to pull content up */}
            ...
            <div className="flex flex-wrap items-center gap-x-3 sm:gap-x-4 gap-y-1.5 my-3 sm:my-4 text-xs sm:text-sm">
                {film.movie.status && <h6 className="text-white border border-gray-600 p-1 px-1.5 rounded">{film.movie.status === "ongoing" ? "Đang cập nhật" : "Hoàn Thành"}</h6>}
            ...
            {film.movie.content && (
                <p className="text-gray-300 my-4 sm:my-5 text-sm sm:text-base leading-relaxed" dangerouslySetInnerHTML={{ __html: film.movie.content }}></p>
            )}
            
            <div className="space-y-1.5 text-sm sm:text-base text-gray-300">
                {film.movie.director?.length > 0 && (
                    <p><span className="text-gray-400 font-medium">Đạo diễn: </span>{Array.isArray(film.movie.director) ? film.movie.director.join(", ") : film.movie.director}</p>
                )}
                {film.movie.actor?.length > 0 && (
                    <p><span className="text-gray-400 font-medium">Diễn viên: </span>{Array.isArray(film.movie.actor) ? film.movie.actor.slice(0, 6).join(", ") + (film.movie.actor.length > 6 ? ", ..." : "") : film.movie.actor}</p>
                )}
                {film.movie.category?.length > 0 && (
                    <p><span className="text-gray-400 font-medium">Thể loại: </span>{Array.isArray(film.movie.category) ? film.movie.category.map(c => c.name).join(", ") : film.movie.category}</p>
                )}
                {film.movie.country?.length > 0 && (
                    <p><span className="text-gray-400 font-medium">Quốc gia: </span>{Array.isArray(film.movie.country) ? film.movie.country.map(c => c.name).join(", ") : film.movie.country}</p>
                )}
            </div>
            </div>
        </div>
