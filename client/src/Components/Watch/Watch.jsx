...
const Watch = () => {
  ...
  useEffect(() => {
    if (film?.movie?.video_url && videoRef.current) {
      ...
      const updateTime = () => {
        if (!videoRef.current) return; // Guard against null ref on unmount
      ...
      video.addEventListener('loadedmetadata', updateTime); // Ensure duration is set early
  ...
  const handleFastForward = () => {
    if (videoRef.current && duration > 0) {
      ...
      videoRef.current.currentTime = newTime;
    } else if (videoRef.current) { // Fallback if duration not fully available
    ...
    <div>
      ...
      >
        ...
        {film.movie.video_url ? (
          <div ref={videoContainerRef} className={`absolute inset-0 mt-28 mx-auto lg:w-[85%] lg:h-[90%] h-[80%] w-[100%] flex flex-col justify-center items-center ${isFullscreen ? 'fullscreen' : ''}`}>
            <video
              ref={videoRef}
            ...
            <div className="absolute bottom-0 left-0 w-full bg-black bg-opacity-70 text-white p-2 flex items-center">
              {/* Rewind Button */}
              <button onClick={handleRewind} className="mr-2" title="Rewind 10s">
                <i className='bx bx-rewind text-2xl'></i>
              </button>

              <button onClick={togglePlay} className="mr-2" title={isPlaying ? "Pause" : "Play"}>
                <i className={`bx ${isPlaying ? 'bx-pause' : 'bx-play'} text-2xl`}></i>
              </button>
              
              {/* Fast Forward Button */}
              <button onClick={handleFastForward} className="mr-2" title="Forward 10s">
                <i className='bx bx-fast-forward text-2xl'></i>
              </button>
              
              <button onClick={toggleMute} className="mr-1" title={isMuted ? "Unmute" : "Mute"}>
              ...
              <div className="text-sm mr-2 whitespace-nowrap">{formatTime(currentTime)} / {formatTime(duration)}</div>
              <div 
              ...
              {qualityLevels.length > 1 && (
                <div className="relative mr-2">
                  <button onClick={toggleQualityOptions} className="text-sm" title="Change quality">
                    {currentLevel === -1 ? 'Auto' : (qualityLevels[currentLevel]?.height ? `${qualityLevels[currentLevel]?.height}p` : 'SD')} <i className="bx bx-chevron-down"></i>
                  </button>
                  {showQualityOptions && (
                    <div className="absolute bottom-full right-0 bg-black bg-opacity-80 p-2 rounded z-10 whitespace-nowrap">
                      <button onClick={() => setVideoLevel(-1)} className={`block w-full text-left text-sm hover:bg-gray-700 p-1 ${currentLevel === -1 ? 'font-bold' : ''}`}>
                        Tự động
                      </button>
                      {qualityLevels.map((level, index) => (
                        <button key={index} onClick={() => setVideoLevel(index)} className={`block w-full text-left text-sm hover:bg-gray-700 p-1 ${currentLevel === index ? 'font-bold' : ''}`}>
              ...
              <button onClick={toggleFullscreen} title={isFullscreen ? "Exit Fullscreen" : "Fullscreen"}>
                <i className={`bx ${isFullscreen ? 'bx-exit-fullscreen' : 'bx-fullscreen'} text-2xl`}></i>
              </button>
            </div>
          </div>
        ) : (
        ...
        <div className="container mx-auto max-w-screen-xl p-3">
          <div>
            ...
            <h1 className="text-xl text-gray-300 font-light my-2 block">
              ...
              <h3 className="text-red-700 inline font-medium ml-2">({film.movie.year})</h3>
            </h1>
            ...
            <p className="text-white my-5" dangerouslySetInnerHTML={{ __html: film.movie.content }}></p>
            ...
            <h3 className="text-white">
              ...
              {Array.isArray(film.movie.actor) ? (film.movie.actor.slice(0, 4).join(', ') +
                (film.movie.actor.length > 4 ? '...' : '')) : film.movie.actor}
            </h3>
            <h3 className="text-white">
              <span className="text-red-700 font-semibold">Thể loại: </span>
              {Array.isArray(film.movie.category) ? film.movie.category.map(c => c.name).join(', ') : (film.movie.genre || 'N/A')}
            </h3>
          </div>
        </div>
      </div>
