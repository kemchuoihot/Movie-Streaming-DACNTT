...
const Watch = () => {
  ...
  useEffect(() => {
    const incrementView = async () => {
      try {
        await axios.put(`http://localhost:5000/api/movies/${slug}/view`, {}, {
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-all duration-500  ${
      isLightsOff ? 'bg-black' : 'bg-gradient-to-br from-slate-900 via-purple-900/20 to-slate-900'
    }`}>
      ...
      <main className="flex-grow">
        {film && (
          <>
            ...
            <div className={`relative ${isLightsOff ? 'bg-black' : 'bg-gradient-to-b from-slate-900/50 to-slate-900'}`}>
                ...
                {film.movie.video_url ? (
                  <div
                    ...
                    className={`group relative mx-auto w-[98%] sm:w-[95%] md:w-[90%] lg:w-[85%] aspect-video flex flex-col justify-center items-center rounded-2xl overflow-hidden shadow-2xl bg-black border-2 border-white/10 ${
                      isFullscreen ? "fixed top-0 left-0 w-full h-full z-[5000] rounded-none !m-0 !max-h-full !max-w-full border-none" : ""
                    }`}
                  ...
                  >
                    ...
                    {/* Enhanced Controls */}
                    <div className={`absolute bottom-0 left-0 w-full bg-gradient-to-t from-black/90 via-black/60 to-transparent text-white p-3 sm:p-4 flex flex-col transition-all duration-300 ${
                      ...
                      <div className="w-full flex items-center justify-between px-2">
                        ...
                        <div className="flex items-center gap-2">
                          <button onClick={handleRewind} className="p-2 hover:bg-white/20 rounded-full transition-all duration-200" title="Tua lùi 10s">
                            <i className="bx bx-rewind text-xl sm:text-2xl"></i>
                          </button>
                          <button onClick={togglePlay} className="p-2 hover:bg-white/20 rounded-full transition-all duration-200" title={isPlaying ? "Tạm dừng" : "Phát"}>
                            <i className={`bx ${isPlaying ? "bx-pause" : "bx-play"} text-2xl sm:text-3xl`}></i>
                          </button>
                          <button onClick={handleFastForward} className="p-2 hover:bg-white/20 rounded-full transition-all duration-200" title="Tua tiến 10s">
                            <i className="bx bx-fast-forward text-xl sm:text-2xl"></i>
                          </button>
                          
                          {/* Volume Controls */}
                          <div className="flex items-center gap-2 ml-3">
                            <button onClick={toggleMute} className="p-2 hover:bg-white/20 rounded-full transition-all duration-200" title={isMuted ? "Bật tiếng" : "Tắt tiếng"}>
                              <i className={`bx ${volumeIconClass} text-xl sm:text-2xl`}></i>
                            </button>
                            <input
                              type="range"
                              min="0"
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          >
                            <i className={`bx ${isLightsOff ? 'bx-bulb' : 'bx-moon'} text-xl`}></i>
                          </button>
                                ...
                                {speedOptionsList.map((rate) => (
                                  <button
                                    ...
                                    className={`block w-full text-left text-sm hover:bg-white/20 p-2 rounded transition-all duration-200 ${
                                      playbackRate === rate ? "bg-red-500/20 text-red-400 font-bold" : ""
                                    }`}
                                  >
                              ...
                              {showQualityOptions && (
                                <div className="absolute bottom-full mb-2 right-0 bg-black/90 backdrop-blur-sm p-2 rounded-xl z-30 shadow-2xl border border-white/20">
                                  <button
                                    ...
                                    className={`block w-full text-left text-sm hover:bg-white/20 p-2 rounded transition-all duration-200 ${
                                      currentLevel === -1 ? "bg-red-500/20 text-red-400 font-bold" : ""
                                    }`}
                                  >
                                    Tự động
                                  </button>
                                  {qualityLevels.map((level, index) => (
                                    <button
                                      ...
                                      className={`block w-full text-left text-sm hover:bg-white/20 p-2 rounded transition-all duration-200 ${
                                        currentLevel === index ? "bg-red-500/20 text-red-400 font-bold" : ""
                                      }`}
                                    >
                  ...
                  <div className="mx-auto w-[98%] sm:w-[95%] md:w-[90%] lg:w-[85%] aspect-video flex flex-col items-center justify-center text-white rounded-2xl shadow-2xl bg-gradient-to-br from-gray-800 to-gray-900 border border-white/10">
                    ...
                    <p className="text-xl font-medium">Không có video để phát</p>
                    <p className="text-sm text-gray-400 mt-2">Phim này hiện chưa có nguồn phát video</p>
            ...
            <div className={`relative pb-12 transition-all duration-500 ${
              isLightsOff ? 'bg-black opacity-30' : 'bg-gradient-to-br from-slate-900 via-gray-800 to-slate-900'
            }`}>
              <div className="container mx-auto max-w-screen-xl p-4 md:p-6">
                <div className="bg-white/10 backdrop-blur rounded-3xl p-6 md:p-8 border border-white/20 shadow-2xl">
                  ...
                  {film.movie.content && (
                    <div className="mb-8">
                      <h3 className="text-xl font-semibold text-white mb-3">Nội dung phim</h3>
