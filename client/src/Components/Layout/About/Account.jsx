    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        try {
          ...
          setUser({ email: response.data.email, username: response.data.displayName || 'N/A' });
  ...
  return (
    <div className="min-h-screen bg-[#06121e] p-8">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl text-white font-semibold mb-6">Quản lý tài khoản</h1>
            ...
            <>
              <div className="flex space-x-4 mb-6">
                <button
                  ...
                  className={`px-4 py-2 rounded-md ${activeTab === 'info' ? 'bg-[#153a61]' : 'hover:bg-[#153a61]'}`}
                >
                  Thông tin
                </button>
                <button
                  onClick={() => setActiveTab('users')}
                  className={`px-4 py-2 rounded-md ${activeTab === 'users' ? 'bg-[#153a61]' : 'hover:bg-[#153a61]'}`}
                >
                  Quản lý người dùng
                </button>
                <button
                  onClick={() => setActiveTab('movies')}
                  className={`px-4 py-2 rounded-md ${activeTab === 'movies' ? 'bg-[#153a61]' : 'hover:bg-[#153a61]'}`}
                >
                  Quản lý phim
                </button>
              </div>
              {activeTab === 'info' && (
                <div>
                  ...
                  <p><strong>Tên người dùng:</strong> {user.username || 'N/A'}</p>
                ...
                <>
                  <form onSubmit={handleUserSubmit} className="mb-6">
                    ...
                    <button
                      type="submit"
                      disabled={loading}
                      className={`mt-4 w-full bg-indigo-600 text-white py-2 rounded-lg hover:bg-indigo-700 ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
